...
import { analyzePineScript, validateShortTitle, initializeParser } from '../parser/index.js';
...
export async function integrateWithExistingValidation(code, existingViolations, validationRules) {
  ...
  for (const astViolation of astViolations) {
    ...
    const existingViolation = existingViolations.find(v => 
      v.line === astViolation.line && 
      v.rule === astViolation.rule
    );
...
export class IntegrationPerformanceMonitor {
  ...
  getPerformanceReport() {
    return {
      ...
      targetDuration: 15,
      recommendations: this.alerts.length > 0 ? [
...
export class ValidationRulesManager {
  ...
  async loadRules(rulesPath) {
    ...
    if (this.rules && this.lastLoaded && 
        (Date.now() - this.lastLoaded) < this.cacheValidMs) {
      return this.rules;
    }
...
export function validateBackwardCompatibility(oldViolations, newViolations) {
  ...
  for (const oldViolation of oldViolations) {
    const newViolation = newViolations.find(v => 
      v.line === oldViolation.line && 
      v.rule === oldViolation.rule
    );
  ...
  for (const newViolation of newViolations) {
    const oldViolation = oldViolations.find(v => 
