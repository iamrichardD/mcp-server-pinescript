...
async function testSyntaxCompatibility() {
  ...
  const start = process.hrtime.bigint();
  const result = validateSyntaxCompatibility(`//@version=6\nstrategy('Complex Test', shorttitle='CT', overlay=false)`);
  const end = process.hrtime.bigint();
...
async function runComprehensiveTests() {
  ...
  console.log(`📊 Data Foundation: ${dataTest.functionsMatch && dataTest.variablesMatch ? '✅ PASS' : '❌ FAIL'}`);
  console.log(`🔍 Search Performance: ${searchTest.performanceOk ? '✅ PASS' : '❌ FAIL'} (${searchTest.searchTime.toFixed(3)}ms)`);
  console.log(`🔧 Validation Performance: ${validationTests.every(t => t.performanceOk) ? '✅ PASS' : '❌ FAIL'}`);
  console.log(`⚙️  Syntax Compatibility: ${syntaxTest.performanceOk ? '✅ PASS' : '❌ FAIL'} (${syntaxTest.compatibilityTime.toFixed(3)}ms)`);
  console.log(`🎯 Overall Performance: ${allPerformanceOk ? '✅ PASS - All <15ms' : '❌ FAIL - Some >15ms'}`);
  ...
  console.log(`SHORT_TITLE_TOO_LONG detection: ${shortTitleTest?.issueCount > 0 ? '✅ WORKING' : '❌ NOT WORKING'}`);
  console.log(`INVALID_PRECISION detection: ${precisionTest?.issueCount > 0 ? '✅ WORKING' : '❌ NOT WORKING'}`);
