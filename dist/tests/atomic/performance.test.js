...
import { analyzePineScript, quickValidateShortTitle, PerformanceMonitor, getParserStatus, } from '../../src/parser/index.js';
import { success, error, isSuccess, createError, ErrorCollector, tryParse, combineResults, ERROR_CODES, } from '../../src/parser/error-handler.js';
class PerformanceBenchmarker {
    benchmarks = [];
    async measureOperation(operation, target, fn) {
    ...
    measureSync(operation, target, fn) {
        const memoryBefore = this.getMemoryUsage();
        const start = performance.now();
        const result = fn();
        const duration = performance.now() - start;
...
describe('Performance Validation Framework - Integration Performance', () => {
    ...
    describe('Main Parser API Performance', () => {
        it('should meet <15ms parsing target for simple scripts', async () => {
            ...
            const { benchmark } = await benchmarker.measureOperation('analyzePineScript() simple', 15.0, // <15ms target
      ...
      `;
            const { benchmark } = await benchmarker.measureOperation('analyzePineScript() medium', 15.0, () => analyzePineScript(mediumScript));
            expect(benchmark.passed).toBe(true);
        });
        it('should meet <5ms validation target', async () => {
            const validScript = 'indicator("Valid", "V", overlay=true)';
            const { benchmark } = await benchmarker.measureOperation('quickValidateShortTitle()', 5.0, // <5ms target
        ...
        it('should handle error cases within performance targets', async () => {
            ...
            const { benchmark } = await benchmarker.measureOperation('analyzePineScript() error handling', 15.0, // Should still meet target even with errors
    ...
    describe('Batch Operations Performance', () => {
        it('should maintain performance across multiple operations', async () => {
            ...
            const { benchmark } = await benchmarker.measureOperation('Batch analysis (5 scripts)', 50.0, // 5 scripts * ~10ms each
            async () => {
                const results = await Promise.all(scripts.map(script => analyzePineScript(script)));
                return results;
            });
            expect(benchmark.passed).toBe(true);
        });
        it('should validate sequential performance consistency', async () => {
            ...
            for (let i = 0; i < iterations; i++) {
                const { benchmark } = await benchmarker.measureOperation(`Sequential analysis ${i + 1}`, 15.0, () => analyzePineScript(script));
...
describe('Performance Validation Framework - Memory Usage Monitoring', () => {
    ...
    describe('Memory Efficiency Validation', () => {
      ...
      `;
            const { benchmark } = await benchmarker.measureOperation('Memory usage analysis', 15.0, () => analyzePineScript(largerScript));
            expect(benchmark.passed).toBe(true);
        ...
        it('should validate error collector memory efficiency', () => {
            const collector = new ErrorCollector();
            const { benchmark } = benchmarker.measureSync('Error collector bulk operations', 5.0, () => {
                // Add many errors
...
describe('Performance Validation Framework - Regression Detection', () => {
    ...
    describe('Performance Baseline Validation', () => {
        it('should establish and validate performance baselines', async () => {
            ...
            for (const testCase of testCases) {
                const { benchmark } = await benchmarker.measureOperation(`Baseline: ${testCase.script.slice(0, 20)}...`, testCase.target, () => analyzePineScript(testCase.script));
                expect(benchmark.passed).toBe(true);
            }
        });
        it('should detect performance regressions', async () => {
            ...
            // Run baseline
            const { benchmark: baseline } = await benchmarker.measureOperation('Regression baseline', 15.0, () => analyzePineScript(script));
            // Run follow-up tests
            for (let i = 0; i < 5; i++) {
                const { benchmark } = await benchmarker.measureOperation(`Regression test ${i + 1}`, baseline.actual + acceptableVariance, () => analyzePineScript(script));
                expect(benchmark.passed).toBe(true);
            }
        });
    });
    describe('Performance Summary and Reporting', () => {
        it('should generate performance summary', async () => {
            ...
            await benchmarker.measureOperation('Summary test 1', 10.0, () => analyzePineScript('indicator("S1", "S1")'));
            benchmarker.measureSync('Summary test 2', 1.0, () => success('test'));
            benchmarker.measureSync('Summary test 3', 0.5, () => createError(ERROR_CODES.INVALID_TOKEN, 'test'));
