...
import { success, error, isSuccess, isError, createError, createLexicalError, createSyntaxError, createShortTitleError, tryParse, tryParseAsync, combineResults, ERROR_CODES, ERROR_SEVERITY, ERROR_CATEGORIES, RECOVERY_STRATEGIES, } from '../../src/parser/error-handler.js';
class AtomicPerformanceValidator {
    results = [];
    measure(testName, targetMs, category, fn) {
    ...
    async measureAsync(testName, targetMs, category, fn) {
        const memoryBefore = this.getMemoryUsage();
        const start = performance.now();
...
describe('Atomic Testing Framework - Core Result Pattern', () => {
    describe('Ultra-Fast Result Operations (<0.5ms)', () => {
        it('success() creation should be atomic fast', () => {
            const measurement = validator.measure('success() creation', 0.5, 'result-pattern', () => success('atomic test data'));
        ...
        it('error() creation should be atomic fast', () => {
            ...
            const measurement = validator.measure('error() creation', 0.5, 'result-pattern', () => error(testError));
            expect(measurement.passed).toBe(true);
        ...
        it('isSuccess() validation should be ultra-fast', () => {
            ...
            const measurement = validator.measure('isSuccess() check', 0.2, 'type-guards', () => isSuccess(testResult));
            expect(measurement.passed).toBe(true);
            expect(measurement.result).toBe(true);
        });
        it('isError() validation should be ultra-fast', () => {
            const testResult = error('validation test');
            const measurement = validator.measure('isError() check', 0.2, 'type-guards', () => isError(testResult));
            expect(measurement.passed).toBe(true);
            expect(measurement.result).toBe(true);
        });
    });
...
describe('Atomic Testing Framework - Error Creation System', () => {
    describe('Error Factory Performance (<1ms)', () => {
        it('createError() basic should meet atomic target', () => {
            const measurement = validator.measure('createError() basic', 1.0, 'error-creation', () => createError(ERROR_CODES.INVALID_TOKEN, 'Atomic test error'));
            expect(measurement.passed).toBe(true);
        ...
        it('createError() with options should meet target', () => {
            const measurement = validator.measure('createError() complex', 1.5, 'error-creation', () => createError(ERROR_CODES.TYPE_MISMATCH, 'Complex error', undefined, {
        ...
        it('createLexicalError() should be efficient', () => {
            ...
            const measurement = validator.measure('createLexicalError()', 1.0, 'error-creation', () => createLexicalError('Invalid atomic character', location, 'Remove invalid character'));
            expect(measurement.passed).toBe(true);
        ...
        it('createSyntaxError() should be efficient', () => {
            ...
            const measurement = validator.measure('createSyntaxError()', 1.0, 'error-creation', () => createSyntaxError(')', '(', location));
            expect(measurement.passed).toBe(true);
        ...
        it('createShortTitleError() should be efficient', () => {
            ...
            const measurement = validator.measure('createShortTitleError()', 1.5, 'error-creation', () => createShortTitleError('This atomic test title is too long', 34, 20, location));
            expect(measurement.passed).toBe(true);
...
describe('Atomic Testing Framework - Utility Functions', () => {
    describe('Error Handling Utilities (<2ms)', () => {
        it('tryParse() success should be fast', () => {
            const measurement = validator.measure('tryParse() success', 1.0, 'utilities', () => tryParse(() => 'atomic success operation'));
            expect(measurement.passed).toBe(true);
        ...
        it('tryParse() error handling should be fast', () => {
            const measurement = validator.measure('tryParse() error', 1.5, 'utilities', () => tryParse(() => { throw new Error('atomic test error'); }));
            expect(measurement.passed).toBe(true);
        ...
        it('tryParseAsync() should meet async target', async () => {
            const measurement = await validator.measureAsync('tryParseAsync() success', 5.0, 'utilities', () => tryParseAsync(async () => Promise.resolve('atomic async success')));
            expect(measurement.passed).toBe(true);
            expect(isSuccess(measurement.result)).toBe(true);
            if (isSuccess(measurement.result)) {
        ...
        it('combineResults() should be efficient', () => {
            ...
            const measurement = validator.measure('combineResults() batch', 1.0, 'utilities', () => combineResults(results));
            expect(measurement.passed).toBe(true);
            expect(isSuccess(measurement.result)).toBe(true);
            if (isSuccess(measurement.result)) {
...
describe('Atomic Testing Framework - Constants and System', () => {
    describe('Constant Access Performance (<0.1ms)', () => {
        it('ERROR_CODES access should be instant', () => {
            const measurement = validator.measure('ERROR_CODES access', 0.1, 'constants', () => {
                return [
        ...
        it('ERROR_SEVERITY access should be instant', () => {
            const measurement = validator.measure('ERROR_SEVERITY access', 0.1, 'constants', () => Object.values(ERROR_SEVERITY));
            expect(measurement.passed).toBe(true);
            expect(measurement.result).toEqual(['info', 'warning', 'error', 'critical']);
        });
        it('ERROR_CATEGORIES access should be instant', () => {
            const measurement = validator.measure('ERROR_CATEGORIES access', 0.1, 'constants', () => Object.values(ERROR_CATEGORIES));
            expect(measurement.passed).toBe(true);
        ...
        it('RECOVERY_STRATEGIES access should be instant', () => {
            const measurement = validator.measure('RECOVERY_STRATEGIES access', 0.1, 'constants', () => Object.values(RECOVERY_STRATEGIES));
            expect(measurement.passed).toBe(true);
    ...
    describe('High-Volume Performance Testing', () => {
        it('should maintain atomic performance under load', () => {
            ...
            for (let i = 0; i < iterations; i++) {
                const measurement = validator.measure(`batch-success-${i}`, 0.5, 'load-testing', () => success(`data-${i}`));
        ...
        it('should handle complex operations efficiently', () => {
            ...
            for (let i = 0; i < iterations; i++) {
                const measurement = validator.measure(`complex-operation-${i}`, 2.0, 'load-testing', () => {
