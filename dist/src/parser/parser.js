 ...
 */
import { AST_NODE_TYPES, createFunctionCallNode, createLiteralNode, createParameterNode, createSourceLocation, } from './ast-types.js';
    ...
    if (check(parser, TOKEN_TYPES.IDENTIFIER) || check(parser, TOKEN_TYPES.KEYWORD)) {
        ...
        if (expr.type === 'MemberExpression' && check(parser, TOKEN_TYPES.LBRACKET)) {
            parser.errors.push({
                ...
                message: "Cannot use the history-referencing operator on fields of user-defined types. Reference the history of the object first (e.g., '(object[1]).field').",
                location: getCurrentLocation(parser),
                severity: 'error',
            });
