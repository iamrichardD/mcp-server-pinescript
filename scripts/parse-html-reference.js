...
class PineScriptHTMLParser {
  ...
  parseStyleGuide(document) {
    ...
    for (const heading of headings) {
      ...
      if (sectionData && sectionData.id !== "style-guide") {
        // Skip main title
  ...
  determineSeverity(title, rules) {
    ...
    if (
      ...
      rulesText.includes("must") ||
      rulesText.includes("required")
    ) {
      return "error";
    } else if (
      rulesText.includes("recommend") ||
      rulesText.includes("should") ||
  ...
  saveStyleRules(outputDir) {
    ...
    console.log(
      `   🎨 ${this.getStyleRuleCategories().naming + this.getStyleRuleCategories().structure + this.getStyleRuleCategories().formatting} total style rules`
    );
    console.log(
      `   🚀 ${Math.round((this.styleRules.size / 2) * 100)}% coverage improvement over basic rules`
    );
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const htmlFile =
    process.argv[2] || path.join(__dirname, "../docs/v6/__pine-script-reference.html");
  ...
  const processStyleGuide =
    process.argv.includes("--style-guide") || htmlFile.includes("style-guide");
