...
class PineScriptDocProcessor {
  ...
  async updateDocumentation() {
    ...
    console.log(
      "⏳ Note: This process includes 5-second delays between requests to respect TradingView servers"
    );
  ...
  validateScrapedContent(content, url) {
    ...
    if (content.includes("NoSuchKey") || content.includes("AccessDenied")) {
      issues.push("AWS S3 error detected");
    }

    // Check for TradingView "page not found" messages
    if (
      content.includes("This isn't the page you're looking for") ||
  ...
  generateQualityReport() {
    ...
    if (this.qualityStats.totalAttempts > 0) {
      const successRate = (
        (this.qualityStats.validContent / this.qualityStats.totalAttempts) *
        100
      ).toFixed(1);
  ...
  async processStyleGuideContent(markdown) {
    ...
    // Save raw markdown
    await fs.writeFile(path.join(rootDir, "docs", this.version, filename), markdown);
