...
plot(slowEma)
      ...
      const typeError = result.violations.find(
        (v) =>
...
plot(macdLine)
      ...
      const typeErrors = result.violations.filter(
        (v) => v.rule === "SERIES_TYPE_WHERE_SIMPLE_EXPECTED" && v.message.includes("ta.macd")
      );
...
plot(slowEma)
      ...
      const conversionError = result.violations.find(
        (v) =>
          v.rule === "SERIES_TYPE_WHERE_SIMPLE_EXPECTED" &&
...
strategy.entry("Long", strategy.long, qty = position.positionSize)
      ...
      const qtyError = result.violations.find(
        (v) =>
          v.rule === "SERIES_TYPE_WHERE_SIMPLE_EXPECTED" &&
...
atr = ta.atr(indicators.atrLength)
      ...
      const typeErrors = result.violations.filter(
        (v) => v.rule === "SERIES_TYPE_WHERE_SIMPLE_EXPECTED"
      );
