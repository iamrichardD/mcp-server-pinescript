...
describe("Performance Requirements", () => {
  ...
  describe("Scalability Performance", () => {
    ...
    it("should handle large scripts (500 lines) in <200ms", () => {
      const largeSource = Array(500)
        .fill(0)
        .map((_, i) => `var${i} = ta.sma(close, ${(i % 50) + 5})`)
        .join("\n");
    ...
    it("should validate multiple SHORT_TITLE_TOO_LONG errors efficiently", async () => {
      const multipleErrors = Array(20)
        .fill(0)
        .map((_, i) => `indicator("Test ${i}", shorttitle="VERYLONGNAME${i}")`)
        .join("\n");
  ...
  describe("Comparison with Performance Targets", () => {
    it("should meet MCP server integration requirements (<100ms total)", async () => {
      const source =
        'indicator("Complex Indicator", shorttitle="VERYLONGNAME", overlay=true, precision=4)';
