...
import {
  ...
  isPineScriptParseError as indexIsPineScriptParseError,
  isPineScriptValidationError as indexIsPineScriptValidationError,
...
import {
  ...
  createLiteralNode,
  AST_NODE_TYPES,
} from '../../src/parser/ast-types.js';

// Performance monitoring for atomic tests
function measureTypeGuard<T extends (...args: any[]) => any>(
  guardName: string,
  guard: T,
  ...args: Parameters<T>
): ReturnType<T> {
...
describe('Type Guards - Result Pattern Validation', () => {
  describe('isSuccess() type guard', () => {
    ...
    it('should handle edge cases in <0.5ms', () => {
      ...
      expect(measureTypeGuard('isSuccess fake', isSuccess, fakeSuccess as any)).toBe(true);
      expect(measureTypeGuard('isSuccess malformed', isSuccess, malformedResult as any)).toBe(false);
      expect(measureTypeGuard('isSuccess incomplete', isSuccess, incompleteResult as any)).toBe(true);
    });
    
    it('should handle non-object inputs in <0.5ms', () => {
      expect(measureTypeGuard('isSuccess string', isSuccess, 'not an object' as any)).toBe(false);
      expect(measureTypeGuard('isSuccess number', isSuccess, 42 as any)).toBe(false);
      expect(measureTypeGuard('isSuccess null', isSuccess, null as any)).toBe(false);
      expect(measureTypeGuard('isSuccess undefined', isSuccess, undefined as any)).toBe(false);
    });
  });
  ...
  describe('isError() type guard', () => {
    ...
    it('should handle edge cases in <0.5ms', () => {
      ...
      expect(measureTypeGuard('isError fake', isError, fakeError as any)).toBe(true);
      expect(measureTypeGuard('isError malformed', isError, malformedError as any)).toBe(false);
      expect(measureTypeGuard('isError incomplete', isError, incompleteError as any)).toBe(true);
    });
  });
...
describe('Type Guards - Error Class Validation', () => {
  describe('isPineScriptParseError() type guard', () => {
    it('should validate quickly', () => {
      ...
      const parseErrorWithLocation = new PineScriptParseError(
        'Parse error with location',
        createSourceLocation(1, 5, 5, 3),
        'CUSTOM_CODE'
      );
    ...
    it('should reject other error types in <0.5ms', () => {
      ...
      const customError = new (class CustomError extends Error {})('Custom error');
  ...
  describe('isPineScriptValidationError() type guard', () => {
    it('should validate quickly', () => {
      const validationError = new PineScriptValidationError('Validation error', []);
      const validationErrorWithViolations = new PineScriptValidationError(
...
describe('Type Guards - AST Node Validation', () => {
  ...
  describe('isFunctionCallNode() type guard', () => {
    it('should validate quickly', () => {
      const location = createSourceLocation(1, 0, 0, 5);
      const functionCall = createFunctionCallNode('indicator', [], location);
      const functionCallWithParams = createFunctionCallNode(
        'plot',
        [createParameterNode(createLiteralNode('close', location, 'close'), location)],
        location,
        'ta'
      );
  ...
  describe('Type guard composition', () => {
    it('should work correctly when composed in <0.5ms', () => {
      ...
      for (const result of results) {
        ...
        } else if (isError(result as any)) {
          if (isPineScriptParseError((result as any).error)) {
            expect((result as any).error).toBeInstanceOf(PineScriptParseError);
          } else if (isPineScriptValidationError((result as any).error)) {
            expect((result as any).error).toBeInstanceOf(PineScriptValidationError);
          } else {
